...
//            System.setProperty("javax.xml.parsers.DocumentBuilderFactory", "com.sun.org.apache.xerces.internal.jaxp.DocumentBuilderFactoryImpl");
    ...
    private Properties loadFromEnvProperties(String propFileName) {
        ...
        try {
            prop = new Properties();
            InputStream in = WebUIBase.class.getClassLoader().getResourceAsStream(
                    propFileName);
            prop.load(in);
            in.close();
        } catch (IOException ioex) {
            ioex.printStackTrace();
            logger.error("配置文件加载失败，请检查是否存在" + propFileName + "文件!");
        }
    ...
    protected void waitBySleepWay(int second) {
        try {
